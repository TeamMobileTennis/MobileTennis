...
public class ServerHandler extends Thread{
    ...
    private void handleCommands(String message)throws Exception{
        ...
        if (!cmd.isEmpty()) {
            switch (cmd) {
                case Constants.CMD.GETINFO:
                    sendMessage(gameLobby.getInformation().toString());
                    break;
                case Constants.CMD.CONN:
                    ...
                    if (Messages.getDataMap(message).size() <= 1) {
                        ...
                        if (!strCode.isEmpty()) {
                            ...
                            if (!handleClose(code)) {
                                ...
                                view.passMessage(addPlayerCode(Messages.getDataStr(Constants.CMD.CLOSE)));
                            }
    ...
    private String addPlayerCode(String message){
        if(Messages.getCommand(message).isEmpty()){
            ...
            return Messages.getDataStr(Constants.CMD.OTHER, Constants.MESSAGE,message, Constants.PLAYER_CODE,Integer.toString(gameLobby.getPlayerPos(this)));
        }else {
            ...
            map.put(Constants.PLAYER_CODE,Integer.toString(gameLobby.getPlayerPos(this)));
